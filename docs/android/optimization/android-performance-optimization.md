...
内存优化一方面避免发生内存泄漏(JVM基础知识)，一方面注意内存的管理([Manage your app's memory](https://developer.android.com/topic/performance/memory))。
...
因为静态变量生命周期等于应用程序的生命周期，所以静态变量引用的变量不会被回收掉。这里涉及到GC Roots的概念。  
...
[Android线程与线程池](/android/framework/android-thread-and-thread-pool/)  
