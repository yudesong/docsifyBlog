...
url: https://mp.weixin.qq.com/s/u2es87JU5FNlGo3qDLY_ng?color_scheme=light
...
![Docker 架构图](https://mmbiz.qpic.cn/sz_mmbiz_png/vI9nYe94fsHib1jDpmfMmcG2GhlsVTWxI0hNDLH4ad65p202gnU8y19bganR0Df4XHl7ewuy8J6hHZ0VW0gZIPQ/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=0)
...
![Docker 和虚拟机对比](https://mmbiz.qpic.cn/sz_mmbiz_png/vI9nYe94fsHib1jDpmfMmcG2GhlsVTWxI16CUA9LI5EPicp6zCxicNkKxyKquRIdMKgibZKlhMb53tOE1hfYAR3gjg/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=1)
...
![Docker Desktop for Mac 安装](https://mmbiz.qpic.cn/sz_mmbiz_png/vI9nYe94fsHib1jDpmfMmcG2GhlsVTWxIE8qOCyj9d86Co8iatFQ4oTINdL8PeAemsFKwd8iapH5kXf0WvktwicjHA/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=2)
...
```
$dockerexec-it nginx-container /bin/bash
```
...
```javascript
$dockerexport-o nginx-container.tar nginx-container
```
...
```bash
FROM nginx:latest
RUN echo 'My Custom Nginx Page' > /usr/share/nginx/html/index.html
EXPOSE 80
```
...
指定容器启动时默认要执行的命令（可以被覆盖）。如果执行`docker run`后面跟启动命令会被覆盖掉。
...
切换执行后续命令的用户和用户组，但这个用户必需首先已使用 RUN 的命令进行创建好了。
...
```
LABELversion="1.0.1"
LABELmaintainer="admin@example.com"
```
...
ENVNODE_VERSION=18.15.0

# 设置多个变量

ENVNODE_VERSION=18.15.0NODE_ENV=production
```
...
ADDproject.tar.gz /app

# 从 URL 下载文件

ADDhttps://example.com/data.json /data
```
...
```
RUNgroupadd -r app && useradd -r -g app appuser
USERappuser
CMD["python","app.py"]
```
...
```markdown
#使用容器名
$dockerexec-it my_container /bin/bash
