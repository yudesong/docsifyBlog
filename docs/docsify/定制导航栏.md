...
那我们可以通过 Markdown 文件来配置导航。首先配置 `loadNavbar`，默认加载的文件为 `_navbar.md`。具体配置规则见[配置项#loadNavbar](#/configuration?id=loadnavbar)。
...
![嵌套导航栏](https://docsify.js.org/_images/zh-cn/nested-navbar.png "嵌套导航栏")

#### 整合自定义导航栏与 emoji 插件

如果你使用 [emoji 插件](#/plugins?id=emoji):
